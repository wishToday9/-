...
1. `std::weak_ptr` 内部包含一个底层的控制块（control block），其中存储有关被 `std::shared_ptr` 管理的对象的引用计数信息。
...
- **原子操作怎么是实现的？** 原子操作的底层实现通常涉及硬件层面的支持，特别是涉及到多核处理器或多处理器系统。原子操作的目标是在多线程环境下保证某个操作的不可分割性，即在执行该操作时不会被其他线程中断。
...
- **对象池和享元模式的区别** 享元模式（Flyweight Pattern）和对象池（Object Pool）都是软件设计中用于提高性能和资源利用率的模式，但它们的关注点和应用场景有所不同。
...
```
开启了alpha test。early-Z是测试完就会更新深度缓存，如果某个片元通过了测试，但是后面alpha test没通过，那这块像素被丢弃，假如该位置上后面的片元都无法通过测试，那就直接G了。
手动修改GPU插值得到的深度
关闭了深度测试
```
...
- **视锥体剔除是怎么实现的？** 图形学中的视锥剔除是一种优化技术，用于排除不在视锥体内的几何体，以提高渲染效率。这个过程通常发生在相机坐标系或裁剪坐标系中。以下是视锥剔除的一般步骤：
...
- **什么是DrawCall**
  ...
  ![img](C++.assets/1689081816366-4963a7b0-8f8d-44d2-9bcc-5df9e5be5773.webp)
...
- **各种测试的含义以及相对顺序**
  ...
  ![img](C++.assets/1689165224077-10845338-4888-42f1-b945-b509513cc905.jpeg)
  ...
  - 模板测试：模板缓冲是用于记录所呈现图元位置的离屏缓存，如下图所示，如果使用了模板缓冲，就相当于在屏幕上有一块模板盖在上面，只有位于这个模板中的图元片段，才会被渲染出来。模板测试就是用片段指定的参考值与模板缓冲中的模板值进行比较，如果达到预设的比较结果，模板测试就通过了，然后用这个参考值更新模板缓冲中的模板值；如果没有达到预设的比较结果，就是没有通过测试，就不更新模板缓冲。简单来说，就是根据物体的位置范围决定是否渲染。
    ...
    ![img](C++.assets/1689165224081-42430013-3268-453b-9131-90a317810a1e.jpeg)
...
* **PBR为什么将diffuse和specular分开**

在基于物理的渲染（Physically Based Rendering，PBR）中，将漫反射（diffuse）和镜面反射（specular）分开是为了更准确地模拟真实光照的物理特性。

1. 漫反射：漫反射是由光线照射到表面上后，均匀地散射到各个方向的过程。漫反射光的强度和颜色不会因为表面的观察方向而变化，而是主要取决于光照方向和表面的法线。漫反射通常用于模拟光照的散射效果，使物体看起来更加柔和和朴实。

2. 镜面反射：镜面反射是光线遇到光滑表面时，按照反射角度完全反射的过程。镜面反射光的强度和颜色会随着观察方向的变化而变化。镜面反射通常用于模拟光的镜面高光效果，使物体表面产生反射。

将漫反射和镜面反射分开的意义在于更好地模拟真实物体的光照行为。在PBR中，通常使用菲涅尔方程来描述材质在不同角度下反射光的强度变化。这种分开的方式允许我们根据物体表面的性质和光照条件，单独控制漫反射和镜面反射的强度和颜色。

此外，将漫反射和镜面反射分开还为材质定义了更多的参数和属性，使其更加灵活。在PBR中，通过调整反射率（albedo）、粗糙度（roughness）和金属度（metalness）等参数，可以更准确地控制材质的光亮度、光滑度和反射效果。

总结起来，将漫反射和镜面反射分开是为了更准确地模拟真实光照的物理特性，并为材质定义更多的参数和属性，以实现更逼真和灵活的渲染效果。



* **什么时候用次表面散射，什么时候用diffuse？**

在PBR中，通常在以下情况下使用次表面散射（Subsurface Scattering，SSS）：

1. 透明或半透明的材质：当材质具有透明性质且传播光线时，光线会在物体内部发生散射并透射回来。这种光线在物体内部的散射过程就是次表面散射。透明或半透明的物体，如人体皮肤、叶子、果冻等，通常需要使用SSS以更真实地模拟光线在物体内部的散射效果。

2. 光线穿透和散射较多的物体：某些物体表面不透明，但对于入射光线会有较多的穿透和散射效果。例如，大理石、蜡烛、牛奶等，具有较高的次表面散射，因此在PBR中可以使用SSS来模拟这些材质的外观。

另一方面，在没有透明性质且不需要模拟光线在物体内部散射的情况下，可以使用单纯的漫反射（diffuse）。这适用于大多数非透明的物体，如金属、木材、塑料等。

重要的是要理解，使用次表面散射会增加渲染的计算成本，因为它需要更复杂的光线传播模拟。因此，根据物体的材质和需求，可以在PBR中选择使用次表面散射或单纯的漫反射来达到所需的视觉效果。
...
游戏绘制中图片压缩不可以使用jpg等的压缩算法，因为算法复杂并且不能拿取特定位置的数据 游戏中基本使用基于块的压缩算法:PC-BC7,DXTC;Mobile:ASTC,ETC,PVRTC
...
因此为了加速这一过程,我们认为在shadow map中着色点x的位置和间接光源x_p的距离可以近似为它们在世界空间中的距离。所以我们认为，对着色点x影响大的间接光源在shadow map中一定也是接近的。
...
那么求 $ E(X^2)$ ,我们就需要额外生成一张shadow map,但是这张图上存的不是depth,而是 depth的平方,然后再在指定范围区域内快速求出平均值,也就是求出了平方值的期望,求出了E(X2),这张存储了depth^2 的shadow map叫做square-depth map.因此需要额外存储一张**square-depth map.**
...
![image-20240107162815334](C++.assets/image-20240107162815334.png)

- 做了一件事：**预处理**。 对于一个数组，先花O(n)的时间走一遍，走的时候就把对应的每一步的累加的和加上。

- **精准定义**：

- - SAT上任何一个元素它表示的都是原本一维数组从最左边的元素加到这个元素的和

- 如何做查询？

- - e.g，上边 **3 7 1 这三个元素的和**可以看做**是前6个的和减去前3个的和**

  - - 前6项、前3项的和，刚刚的预处理都已经算了（上图的下半部分）
    - 那么3 7 1 三元素的和就是20-9=11
...
- **什么样得shader是高质量shader？** 一个好的着色器（shader）通常具有以下特征和品质：
...
延迟渲染把渲染分成了两部，第一步把surface Atrribute绘制到一个Gbuffer中，第二步根据光在Screen Space空间完成光照计算； 第一个问题：G-Buffer在高分辨率下带宽占用是比较大的， 第二个问题：G-Buffer在记录Attribute的时候，在复杂的场景下对于屏幕的一个像素可能会绘制多次，每次绘制都会进行采样、插值等等，再写到G-Buffer中
...
绘制一个10240 X 2048 X 10240的大场景。

基本绘制单元：Plane 16 X 16 (8m x 8m)，每个网格精度是0.5m，中心点在【0，0】，覆盖的范围是(-4-4,-4-4)，一个sector一个 8 x 8个绘制单元 

1. 分为六级LOD，LOD0 Node 一个Node大小为一个Sector，160 X160，LOD5用5X5表示 。

2. 首先建立四叉树，四叉树判断的条件是根据这个Node的中心点位置以及平均高度和摄像机位置的值大小进行判断，计算LOD Node结点中心点位置
   ...
   当满足划分条件的时候，将四个子节点记录到list里面，LOD5判断完以后，将list作为输入，判断lod4的信息，依次完成，直至LOD0，至此。就建立了一个四叉树数据，存放了场景不同位置的Node信息。

3. 在确定完LOD等级以后，然后在Node上进行8*8的划分，划分成更加精细的patch单元，compute sahder group大小为[8,8,1],刚好对应patch，每一个thread计算该patch的中心位置，
   ...
   LOD0里面的一个patch就是一个Plane，其他的就是等比方法Plane得到的图元结构。根据patch生成一个Bound包围盒，完成遮挡剔除和视锥剔除，如果都通过了加入到需要绘制的patch信息列表中，最终经过验证，在不做裁剪的情况下，可以用5000个patch绘制出所有地形，按照原始方法的话原本需要160w个patch，在做完视锥剔除以后，大概减少到2000个，再遮挡剔除后大概再1200左右。

4. 不同LOD等级有一个不同的绘制惊喜密度，在边界的情况下会出现裂缝，然后呢，我就先生成了一个LOD Map 记录了每一个sector的 LOD等级，在处理patch的时候记录下边界处的lod等级之差。在shading的时候，将边界出的这个更加精细的顶点的xz信息坍缩到和LOD等级高的顶点的同一个位置，即完成了一个裂纹的修复。
...
- **是否每一次drawcall都要重新传一次顶点数据**

不是每次draw call 都需要重新传递顶点数据。在一些情况下，可以通过以下方法来避免重新传递顶点数据：

1. Vertex Buffer Objects (VBOs)：VBOs 是一种用于存储顶点数据的缓冲区对象。通过将顶点数据放入 VBO 中，你可以在多次渲染中重用该数据，而不需要每次都重新传递顶点数据。你可以使用函数如 `glBufferData()` 或 `glBufferSubData()` 来填充或更新 VBO 中的数据。

2. Vertex Array Objects (VAOs)：VAOs 定义了一组与渲染相关的状态，包括 VBO、顶点属性指针等。通过使用 VAOs，可以将多个渲染状态绑定到一个 VAO 中，并在渲染时进行快速切换，而无需重新设置顶点数据。

3. Instanced Rendering：如果具有相同顶点数据的对象需要多次渲染，你可以使用 instanced rendering 技术。通过使用实例化顶点属性和 instanced draw call，可以重用相同的顶点数据，并使用不同的变换矩阵或属性来渲染多个实例，而无需重新传递顶点数据。

这些技术可以帮助减少重复传递顶点数据的开销，提高渲染性能。然而，当你需要改变顶点数据时，仍然需要通过 VBO 或其他方法更新顶点数据。

* **OpenGL设置参数那些对渲染效率影响比较大？**

在OpenGL中，设置以下参数可能对性能产生较大的影响：

1. 视口大小（Viewport Size）：视口决定了渲染结果在屏幕上的位置和大小。更大的视口需要渲染更多的像素，可能会导致更多的片段处理和带宽消耗。

2. 渲染目标（Render Targets）：选择渲染到的纹理、帧缓冲或多重采样等渲染目标。渲染到多个渲染目标会增加片段处理的成本。

3. 渲染模式（Rendering Mode）：使用不同的渲染模式，如立体渲染、线框渲染、透明渲染等，可能会对性能产生影响。一些渲染模式可能需要更多的片段处理或导致更多的片段丢弃。

4. 多边形模式（Polygon Mode）：选择填充多边形或仅绘制多边形边缘。填充多边形需要更多的片段进行处理。

5. 深度测试（Depth Testing）和模板测试（Stencil Testing）：开启深度测试或模板测试可以提高渲染质量，但它们需要额外的片段处理来进行深度或模板测试。

6. 抗锯齿（Anti-aliasing）：开启抗锯齿技术可以提高渲染质量，但可能需要更多的计算和带宽消耗。

7. 着色器（Shaders）：复杂的着色器可能会导致较高的片段处理成本。优化着色器代码以减少计算量可能对性能有所帮助。

8. 纹理（Textures）：纹理的大小和数量会影响内存占用和带宽消耗。使用合理的纹理大小和纹理压缩技术可以提高效率。

9. 绘制调用（Draw Calls）：绘制调用的数量会影响CPU到GPU的数据传输和状态切换开销。尽量减少绘制调用，通过合并和批处理减少渲染命令。

这些参数的选择和设置与具体场景和硬件有关，可以根据需要进行调优和优化，以提高OpenGL应用程序的效率。

* **drawcall的性能开销主要在哪一方面？**

draw call 的性能开销主要涉及以下几个方面：

1. CPU 到 GPU 数据传输：每次调用 draw call，需要将顶点数据、纹理数据等传输到 GPU 内存中。频繁的数据传输会增加 CPU 和 GPU 之间的通信开销，尤其是在大规模场景中或者使用大量纹理的情况下。

2. 状态切换（State Change）：每次调用 draw call，需要设置渲染状态，如绑定纹理、着色器、深度测试、模板测试等。频繁的状态切换会导致 GPU 内部状态机的切换，增加了额外的开销。

3. 批次划分（Batching）：每次 draw call 可能只渲染一个物体或者一组物体。如果物体数量较少或散布在多个 draw call 中，会导致较多的绘制调用和状态切换开销。优化的方式是尽量将具有相同材质和渲染状态的物体合并到同一个渲染批次中，以减少绘制调用和状态切换。

4. 顶点和片段处理：每个顶点经过顶点处理器（Vertex Shader）和片段处理器（Fragment Shader）的计算。复杂的顶点和片段处理逻辑会增加 GPU 的计算量，影响性能。

5. Overdraw（过度绘制）：当多个物体重叠或有不透明的片段遮挡住后面的物体时，会发生过度绘制，即同一个像素点被多次绘制。过度绘制会导致不必要的片段处理和带宽消耗，降低性能。

为了优化 draw call 的性能开销，可以采取以下措施：

- 使用批处理技术，将具有相同材质和渲染状态的物体合并到一个渲染批次中。
- 减少状态切换，尽量在一次 draw call 中设置尽可能多的渲染状态。
- 使用硬件实例化技术，重复使用相同的顶点数据，减少数据传输开销。
- 优化顶点和片段处理器的代码，减少计算量。
- 使用裁剪和可见性检测技术，避免不可见的物体的绘制。
- 减少过度绘制，避免在不需要的区域进行绘制。

通过综合考虑和优化这些方面，可以有效地提高 draw call 的性能和整体渲染效率。
...
ImGui（Immediate Mode Graphical User Interface）是一个用于创建用户界面的C++库。它提供了简单易用的接口，允许开发者创建和渲染GUI元素，比如按钮、文本框和滑块等。

要将ImGui绘制到屏幕上，你需要结合一个图形库来实现。通常情况下，ImGui与其他图形库（如OpenGL、DirectX或Vulkan）一起使用。

下面是一个使用OpenGL来绘制ImGui的简单示例：

1. 初始化ImGui库，并设置与图形库相关的上下文：
...
3. 在需要的地方创建ImGui控件，比如按钮或文本框：

```cpp
ImGui::Button("Click Me");
```

4. 在渲染循环的最后，完成ImGui帧的渲染：
...
5. 清理ImGui库和与图形库相关的上下文，在程序结束时释放资源：
...
请注意，这只是一个简单的示例，并且假设你已经配置好了图形库的环境。具体的实现可能会根据你选择的图形库和平台有所不同。在实际使用时，你需要根据你的项目和需求进行相应的配置和编码。

如果你使用的是其他图形库，可以查阅ImGui的官方文档或者寻找适用于该图形库的ImGui集成库、示例代码或教程。根据你选择的图形库，可能会有其他特定的绘制指令或回调函数供你使用。
...
![img](C++.assets/v2-447958dc017b9152f168f7df7cbac5e7_1440w.webp)

​	2. 当V=0时也就是间接光照的情况，这个是我们需要关注与计算的。

![img](C++.assets/v2-abe6005d997b5f06ac9bfd9ccad6dda4_1440w.webp)
...
![img](C++.assets/v2-7924f04b10d02afcb698f829918b305c_720w.webp)
...
- **介绍一下后处理渲染管线** 计算机图形学中的后处理渲染管线是渲染管线的最后一个阶段，用于对渲染输出进行最终的处理和增强。这个阶段通常包括各种效果，如模糊、辉光、色调映射、景深、抗锯齿等，以提高图像的视觉质量或添加特定的艺术效果。以下是后处理渲染管线的一般步骤：
...
[UnityShader*屏幕后处理之运动模糊*unity 关闭运动模糊-CSDN博客](https://blog.csdn.net/weixin_40301728/article/details/107017580#:~:text=思路： 计算获取当前帧的位置以及上一帧的位置，两者做差求出速度 v （方向、大小），利用这个速度我们就可以求得,n 张移动过程中的动画帧，将这些动画帧叠加并取平均，这样就可以得到一个含有 n 个虚影的运动模糊效果。)

1. 叠加图像法。通过采集每一帧的屏幕成像，并将图像累计混合得到模糊运动成像。
2. 第二种方式是通过深度图以及 视角*投影矩阵 变化，得到在当前帧及前一帧的速度变化，直接在当前帧模拟出路径上的模糊残影。 总体上的感觉来说，第一种更为真实，第二种会感觉到较为明显的抖动，尤其是在_DrawCount数量增加 or _BlurSize变大 or _SpeedTime变小 三者有一个或者多个发生的时候，因为此时残影还没画完但物体实际已经偏离了当前位置。
...
1. 基于面片的实现。直接用一个Quat的mesh，加上一张贴图，简单直观的实现。

缺点：只能在平面上贴。

2. 修改贴图。将物体材质的贴图换成原贴图的decal贴图的混合，适用于静态批量的物体。
...
- **洗牌算法**
  ...
  - 证明 Fisher-Yates算法的正确性可以通过数学归纳法来证明。首先，让我们考虑数组中只有两个元素的情况。
...
* **一棵树上很多叶子三角形很多，要怎么节约资源**

在渲染树的场景中，如果要节约资源，可以考虑以下几种方法：

1. 合批处理（Batching）：尽量将具有相同材质和渲染状态的叶子三角形合并成一个单独的渲染批次（batch），这样可以减少渲染调用和状态切换的开销。

2. LOD（层级细节，Level of Detail）：对于离相机较远的叶子，可以使用较低细节的模型，以降低三角形数量。根据距离和屏幕大小，逐渐切换到更简化的模型。

3. 硬件实例化（Hardware Instancing）：如果叶子的模型是相同的，可以使用硬件实例化技术，在GPU端复制和渲染同一个模型的多个实例，从而减少CPU到GPU的数据传输和渲染开销。

4. 裁剪和可见性检测：使用裁剪技术来剔除完全不可见的叶子三角形，以及使用可见性检测算法（如视锥体剔除、遮挡剔除等）来排除在当前相机视野之外的叶子三角形。

5. 纹理压缩和纹理合并：使用纹理压缩算法（如ETC、S3TC等）来减小纹理资源的内存占用，并且可以将多个小纹理合并成一个大纹理，减少纹理切换的开销。

6. 动态LOD和屏幕空间技术：根据叶子三角形的屏幕空间大小和重要性，动态地调整其细节层次，以便在不影响视觉质量的情况下减少三角形数量。

这些是一些常用的优化方法，可以根据具体场景的需求选择适合的方法。同时，还可以结合使用多种优化技术以获得更好的性能和资源利用率。
...
1. **Shader渲染实际是一块一块（fragment）的，有了解吗？**
...
1. **TBN矩阵坐标轴的含义？怎么推导的？**
2. **Hiz遮挡剔除怎么实现mipmap层级确定？降采样怎么实现的？**
...
栈：只要栈的剩余空间大于所申请空间，系统将为程序提供内存，否则将报异常提示栈溢出。
 ...
 堆：首先应该知道操作系统有一个记录空闲内存地址的链表，当系统收到程序的申请时，会遍历该链表，寻找第一个空间大于所申请空间的堆结点，然后将该结点从空闲结点链表中删除，并将该结点的空间分配给程序。 对于大多数系统，会在这块内存空间中的首地址处记录本次分配的大小，这样，代码中的 delete 语句才能正确的释放本内存空间。
...
3. **delete this的作用？(面试官建议好好研究)**

4. **常用的树形结构有哪些？**

5. **红黑树的特点？(问我能不能手撸一个红黑树？)**

6. **多线程和多进程有什么区别？**

   多线程和多进程是并发编程中常用的两个概念，它们都涉及到同时执行多个任务的能力，但有一些关键的区别。

   多线程是指在单个进程内创建多个线程来执行任务。每个线程共享相同的地址空间，文件描述符以及其他进程相关的资源。线程之间可以直接访问彼此的数据，使得线程间通信更加方便快捷。同时，由于线程之间共享同一份资源，多线程的切换开销较小，因此在某些情况下可以提供更高的性能。

   然而，多线程也有一些潜在的问题。由于线程共享内存空间，需要特殊的同步机制来保证数据的一致性，避免出现竞态条件和死锁等问题。另外，一个线程的错误可能会影响整个进程的稳定性，因此对线程的调试和管理也变得更加复杂。

   相比之下，多进程是指在操作系统中创建多个独立的进程来执行任务。每个进程都有自己独立的地址空间和资源，它们通过进程间通信（IPC）来进行数据交换。多进程可以更好地隔离不同任务之间的资源，一个进程的错误通常不会影响其他进程的稳定性。同时，操作系统可以利用多核处理器的并行能力，实现真正的并行执行。

   然而，多进程也存在一些问题。相比于多线程，多进程的切换开销较大，因为需要进行上下文切换和内存重映射。此外，由于进程间的资源独立性，进程间通信的成本较高，需要通过特定的IPC机制进行数据传递和同步。

   综上所述，多线程适用于需要共享数据和信息的任务，并且更适合于IO密集型的应用程序；而多进程适用于需要隔离资源并且可以充分利用多核处理器的计算密集型任务。在实际应用中，需要根据具体的需求和场景选择合适的并发模型。

7. **数据库了解吗？计算机网络了解吗？(不了解)**

8. **空间中有ABC三个点，AB连成线段，如何判断C点在AB的哪一侧？**
...
在计算机图形学中，IBL（Image-Based Lighting）和PBR（Physically Based Rendering）是两个不同但常常结合使用的概念，它们共同致力于实现更真实、基于物理的渲染效果。

**Physically Based Rendering (PBR):** PBR是一种渲染技术，旨在模拟光照和材质的物理行为，以获得更真实的外观。PBR使用物理属性（如金属度、粗糙度、折射率等）来描述材质，使用能量守恒原理来模拟光的反射、折射和散射。这种方法能够产生更真实的光照效果，使得场景中的物体看起来更加自然。

**Image-Based Lighting (IBL):** IBL是一种光照技术，它通过使用全景图或环境贴图来模拟场景中的间接光照。这种方法允许环境的光照信息影响场景中的物体，使得物体的表面反射和阴影更加真实。通常，IBL使用环境贴图作为光照来源，这个贴图包含了场景中的实际光照信息。

结合IBL和PBR的过程通常涉及以下步骤：

- **环境贴图生成：** 预先生成一个全景图或环境贴图，捕捉场景中的光照信息。
- **光照计算：** 使用IBL技术将环境贴图中的光照信息应用到场景中的物体表面。这包括计算间接光照、环境光照和漫反射等。
- **PBR材质模型：** 在PBR中使用物理材质属性（金属度、粗糙度等）来描述物体表面的特性。
- **积分方程求解：** 使用光照方程的数值积分来模拟物体表面的反射、折射和散射，以更准确地计算最终的渲染结果。

通过将这两个技术结合，可以实现更逼真的渲染效果，使得物体表面的反射、折射和阴影更加真实地响应环境中的光照变化。

4. **IBL为PBR贡献了什么？**

Image-Based Lighting (IBL)对Physically Based Rendering (PBR)的贡献主要包括以下方面：

**真实的光照环境：** IBL提供了一种捕捉和应用真实光照环境的方法。通过使用全景图或环境贴图，IBL能够为PBR提供真实世界中的光照信息，使渲染结果更加自然、真实。

**间接光照：** IBL能够捕捉场景中的间接光照信息，即光线在物体表面反射、折射和散射后的效果。这对PBR很重要，因为PBR的目标之一是模拟物体表面对间接光照的响应，而IBL为这一目标提供了有效的工具。

**反射和漫反射：** IBL通过环境贴图可以更准确地模拟物体表面的反射，包括镜面反射和漫反射。PBR使用这些信息来计算材质的反射率和漫反射率，以更真实地呈现光照效果。

**全局光照：** IBL有助于实现全局光照效果，使得场景中的物体能够受到环境中所有光源的影响。这包括来自天空、地面和其他物体的光照，为PBR提供了更细致入微的全局光照计算。

**灵活性和易用性：** IBL使得场景中的光照设置更加灵活和易于调整。通过更改环境贴图，艺术家或开发者可以轻松地改变场景的整体光照氛围，而不必手动调整多个光源的属性。

通过结合IBL和PBR，计算机图形学能够更准确地模拟光照和材质的物理行为，从而生成更真实、更引人入胜的渲染效果。这种结合提高了虚拟场景的视觉质量，使得计算机生成的图像更接近现实世界。
...
5. **随机生成点的算法**
   ...
   * 低差异序列
   ...
   Hammersley 序列，[Holger Dammertz](http://holger.dammertz.org/stuff/notes_HammersleyOnHemisphere.html) 曾仔细描述过它。Hammersley 序列是基于 Van Der Corput 序列，该序列是把十进制数字的二进制表示镜像翻转到小数点右边而得。

   * 泊松随机采样

   整个算法分为大致有以下步骤：

   第一步：设定好两个点之间最近的距离r，以及采样点所在空间的维度n，比如[2维平面](https://www.zhihu.com/search?q=2维平面&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A1039095847})

   第二步：在空间里生成足够多的网格，保证不接触的两个网格之间的点的距离大于r，并且网格数量足够多保证每个网格至多只需装一个采样点就能满足采样数量。为了[最优化](https://www.zhihu.com/search?q=最优化&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A1039095847})，一般取网格边长为$ r/\sqrt{n} $。
   ...
   第四步：设定最小距离[minr](https://www.zhihu.com/search?q=minr&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A1039095847})，比如r，最大距离maxr，比如2*r。以红点为中心生成一个圆环，如下图灰色圆环，在这个圆环中生成一个采样点，如下图蓝点。
...
8. **横竖两次模糊和NXN效果为什么？**
   ...
   二维高斯函数是多元高斯函数的一种特殊形式。它的表达式形式为：

   $$
   f(x, y) = A * exp(-(x^2 / 2σ_x^2 + y^2 / 2σ_y^2))
   $$

   

   其中，A是归一化常数，σ*x和σ*y是标准差。

   通过观察上述表达式，你可以发现，二维高斯函数的指数部分可以拆分成两个独立的一维高斯函数，一个依赖于x，另一个依赖于y。这是因为指数部分的x和y是独立的，它们之间没有交叉项。

   因此，可以将二维高斯函数表示为两个一维高斯函数的乘积：

      f(x, y) = g(x) * h(y)

   其中，

      g(x) = A * exp(-x^2 / 2σ*x^2)   h(y) = exp(-y^2 / 2σ*y^2)

   这样拆分后的两个一维高斯函数分别作用于x和y方向，其效果等同于原始的二维高斯函数。

   这种可分离性的性质使得在图像处理和计算机视觉中，可以通过两个一维高斯滤波器的卷积（在x和y方向上分别进行一维滤波），来实现对图像的二维高斯平滑操作。这种方法在计算上更高效，因为一维滤波器的计算复杂度较低。

   总之，二维高斯函数可以拆分为两个一维高斯函数的乘积，是因为二维高斯函数的指数部分在x和y方向上是独立的，没有交叉项。这种可分离性使得可以使用两个一维高斯函数来近似计算二维高斯函数的效果，并提高计算效率。
...
9. **其他模糊算法**
   ...
   **高斯模糊（Gaussian Blur）：**
   ...
   - **原理：** 高斯模糊是通过对图像中的每个像素应用高斯滤波器来实现的。高斯滤波器是一种权重矩阵，对周围像素进行加权平均。
   - **应用：** 用于平滑图像，减少噪声，以及一些图像预处理和后处理的任务。

   **均值模糊（Mean Blur）：**

   - **原理：** 均值模糊通过对像素周围的邻域进行简单平均来减少图像的细节。
   - **应用：** 与高斯模糊类似，用于图像平滑和去除噪声。

   **运动模糊（Motion Blur）：**

   - **原理：** 运动模糊模拟相机或物体运动时的模糊效果，通过在图像中引入方向性的模糊来实现。
   - **应用：** 用于模拟运动中的相机效果，增加动感。

   **径向模糊（Radial Blur）：**

   - **原理：** 径向模糊通过使图像的中心或某一点成为模糊的轴心，以径向方向模糊图像。
   - **应用：** 用于创造一些艺术效果或在图形设计中产生特殊的视觉效果。

   **景深模糊（Depth of Field Blur）：**

   - **原理：** 景深模糊模拟相机的焦点范围内的清晰，而焦点之外的区域进行模糊处理。
   - **应用：** 用于增加逼真感，特别是在计算机图形学中渲染虚拟场景时。

   **逆深度模糊（Reversed-Z Depth Blur）：**

   - **原理：** 逆深度模糊通常用于游戏图形学中，通过对深度缓冲进行逆深度测试，来模糊前景或近处的物体。
   - **应用：** 用于提高游戏图形的视觉效果，特别是在实时渲染中。

   这些模糊算法可以单独使用或组合使用，具体选择取决于图形效果和应用场景的需求。模糊在图形学中被广泛应用，用于美化、特效和增强图像的表现力。
...
11. **G-Buffer存了都是什么呢？**
    ...
    ![image-20240107154407172](C++.assets/image-20240107154407172.png)

    

12. **V-Buffer算法怎么定位三角形？如何通过图元ID找到对应的Buffer数据？**

13. **Mesh中中的的图元ID都是从0开始，如何确定图元ID怎么对应到哪一个具体Mesh上的哪一个三角形？**
...
17. **PCSS优化(Shadow Masking)**

    因为 PCSS 非常昂贵，那些在半影之中的点才需要用 PCSS，不难想到通过两步来完成。第一个 Pass 做 Pre ShadowMapping 生成一张 Mask 标记半影区域，第二个 Pass 只对半影内的像素做 PCSS 以节省开销。对于其他的点则直接返回 0 或者 1

    因为二值阴影映射只能标记阴影和非阴影，我们还需要想办法生成半影。这里直接抄某原的算法，将原画面 4 x 4 个像素的区域一同计算阴影然后 Down Sample 到 1/4 分辨率的 Mask 贴图：

    ![image-20240107161847001](C++.assets/image-20240107161847001.png)
...
1. unity 行为周期函数
2. 讲讲urp与srp之间的关系
3. GPU Driven的原因是什么呢？为什么要做GPU Driven？
4. 有一些事情是在GPU做不了的？
5. GPU Driven里面比较困难的事情是什么？
6. GPU Driven上比较依赖的API是什么？
7. GPU Driven和传统的方案的区别是什么？
8. 地形最重要的点virtual texture，怎么应用呢？
9. LOD缝隙怎么处理的？
10. 性能有对比吗？(不能从单个LOD减面方面去分析，主要还是在材质采样上)
11. Adaptive virtual texture
12. 怎么在GPU端发起Draw Call呢？
13. 水面渲染怎么实现？
14. 水面模拟怎么实现？
15. 屏幕空间怎么反射？(SSR)
16. urp内置的forwar+管线了解吗？
17. unity内置的反射探针怎么做的？
18. C#是一种托管的语言，C#与C++的区别是什么？
19. C#值类型和引用类型有什么区别？
20. 拆箱和装箱是什么？
21. unity除了渲染这一块吗，有接触过其他的？
22. unity srp batcher底层是怎么实现的？
23. main函数怎么被调用的？
24. stackOverflow是什么？什么时候堆溢出？
25. 什么时候会栈溢出？
26. 栈内存大概有多少？
27. 内存对齐(优化)是谁做的？(操作系统、程序、硬件？)
28. 大小端是什么？
...
![image-20240105195054625](file://C:/Users/zhang/Desktop/Pre/-/C++.assets/image-20240105195054625.png?lastModify=1704433571)
 ...
 \lambda是一个`[0,1]`区间的控制参数，用以在对数和线性之间进行插值。
